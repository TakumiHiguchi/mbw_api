class Api::V1::Mbw::TagController < ApplicationController
  def show
    ...
    if tag
      ...
      tag_hash[:recomanded] = Article.joins(:tags).select('articles.*,tags.*,articles.key AS article_key,tags.key AS tag_key,articles.thumbnail AS articles_thumbnail').create_article_hash({ :search_type => 'tag', :query => tag.name, :limit => 20, :query => nil, :with_thumbnail => true, :with_tag => true })
